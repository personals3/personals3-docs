...
|---|---|---|
| **`Authorization`** | **Yes** | `Bearer <your-key>` — use either a `psk_…` [API key](../account/api-keys.md) or your dashboard JWT |
...
If your file is over a few hundred MB **and** your connection is unreliable, single `PUT` makes a network hiccup wipe out the whole transfer. See [Multipart over the API](./multipart-api.md) for the resumable version.
